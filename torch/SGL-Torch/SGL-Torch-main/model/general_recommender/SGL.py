    ...
    def _forward_gcn(self, norm_adj):
        ...
        for k in range(self.n_layers):
            ...
            if isinstance(norm_adj, list):
                ego_embeddings = torch_sp.mm(norm_adj[k], ego_embeddings)  # 矩阵相乘
            else:
        ...
        user_np = user_embeddings.clone().detach().cpu().numpy()
        # user_np = self.user_embeddings.weight
        # print(len(user_np))
    ...
    def create_adj_mat(self, is_subgraph=False, aug_type='ed'):
        ...
        if is_subgraph and self.ssl_ratio > 0:
            if aug_type == 'nd':
                ...
                # 得到user、item的指示器，0表示drop该节点，1表示不drop
                #  masking vectors: indicator_user indicator_item
                ...
                # user_np_keep 保留的user标签
                # item_np_keep 保留的item标签
    ...
    def train_model(self):
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for bat_users, bat_pos_items, bat_neg_items in data_iter:
                ...
                # L_ssl = L_ssl^user + L_ssl^item
                infonce_loss = torch.sum(clogits_user + clogits_item)

                # 联合任务 Loss
                # L = L_main + lamda1 * L_ssl + lamda2 * theta
